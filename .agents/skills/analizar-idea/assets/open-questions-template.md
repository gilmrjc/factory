...
5. **Responsable**: Quién es responsable de resolver (si aplica)
6. **Plazo**: Cuándo debe resolverse (si aplica)

## Categorías comunes para este skill

Las categorías específicas de `analizar-idea` están definidas en el `SKILL.md` (sección "Preguntas Abiertas — Categorías comunes para este skill"). El template aquí define el formato — no duplica las categorías del skill.

## Gate de avance

La lógica completa del gate (niveles de severidad, estados de avance, flujo, reglas y ejemplos) está en [references/gate-guide.md](../references/gate-guide.md). Consúltala durante la Fase G para decidir `status` y `next`.

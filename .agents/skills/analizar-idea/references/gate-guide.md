...
Lógica completa para evaluar el estado del análisis y decidir `status` y `next` en el frontmatter. Referencia única para el gate de la Fase G — SKILL.md delega aquí.
...
5. Documentar la ejecución del gate en el artefacto (subsección "Gate de avance (Fase G)"):

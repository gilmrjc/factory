---
name: sugerir-test-cases
description: >-
  Analiza código nuevo (plan de implementación o código implementado) e identifica
  casos de prueba faltantes: happy path, edge cases, error conditions, security tests.
  Genera checklist de test cases que deberían implementarse. Salida:
  docs/<domain>/<TICKET-ID>-test-cases.md con matriz de cobertura y
  recomendaciones. Úsalo después de planificar-implementacion (antes de implementar)
  o después de generar-test-strategy para validar casos específicos.
argument-hint: "[STRATEGY-DOC | IMPLEMENTATION-PLAN]"
allowed-tools:
  - read
  - grep
  - find_file_by_name

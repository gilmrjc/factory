---
name: quick-review-xs
description: >-
  Revisión rápida de cambios XS (extra small) con checklist simplificado.
  Valida: funcionalidad, tests, documentación básica, sin breaking changes.
  Salida: OK para merge o recomendaciones. Úsalo después de validar-scope-xs
  para fast-track aprovals (5-10 min vs 30-60 min).
argument-hint: "[TICKET-ID | PR-NUMBER]"
allowed-tools:
  - read
  - grep
  - find_file_by_name
...
# Quick Review para Cambios XS

Revisión rápida (5-10 minutos) de cambios XS. Checklist simplificado vs revisión completa.

Solo análisis: valida cambio es XS-safe, no comprehensive review.
...
**PRECONDICIÓN**: Este skill asume que `validar-scope-xs` ya pasó con OK.

## Fase A — Validar Que Es Realmente XS
...
**If detected**: Change is NOT XS, escalate to full review
```

### ✅ Security Basic (1 minuto)

```
## Security Checklist (XS)
...
- `full-review`: ❌ Escalate to complete `revisar-pr`, not XS anymore
...
### Issues
1. This change exceeds XS scope (5 files, 180 lines)
2. Breaking changes to public API detected
3. Auth changes need architecture review
4. ADR required

→ **NOT XS, use full revisar-pr pipeline**
```

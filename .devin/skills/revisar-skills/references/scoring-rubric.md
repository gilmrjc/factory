...
Cuando overall < 9 o description score < 9:

```plain text
file: <frontmatter | SKILL.md body>
section/line: <referencia>
impact: blocker | important
evidence:
  <excerpt mínimo>
...
Orden: blocker → important. Cuando overall ≥ 9, pon items opcionales bajo Mejoras opcionales (`impact: optional`); no afectan scores.
...
- **4**: Auditoría de resource layout — Checklist + resumen pass/fail
- **5**: Auditoría DRY & assets — Tabla de inventario + checklist + DRY & assets score (1–10) + candidatos extract-shared (`none` si limpio)
- **6**: Auditoría de escritura directa — Tabla de hits grep + checklist + rewrites listos para pegar para hits partial/fail
- **7**: Rewrite de description propuesto — Texto completo si description score < 9; sino `none`
- **8**: Anchors de convención de hermanos — ≥ 2 paths + diferencia de convención específica (no "fases similares")
- **9**: Auditoría de cuerpo — Checklist compartido + ruta de checklist de tipo clasificado citada; cada fila sí/recomendado llenada
- **10**: Puntuación por dimensión + overall — Por dimensión: gate (pass/partial/fail), resumen de cobertura, score; overall = media con [Hard caps](#hard-caps-techos-duros) aplicados
- **11**: Fortalezas — Específicas; vs hermanos
- **12**: Hallazgos — blocker/important cuando overall < 9; Mejoras opcionales cuando overall ≥ 9
...
- Todas las tablas de checklist de auditoría llenadas; clasificación de tipo con justificación registrada
...
- **Q7 Escritura directa**: Tabla de hits grep presente; cada hit partial/fail tiene reemplazo directo listo para pegar en hallazgos

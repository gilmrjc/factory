...
name: revisar-skills
description: >
  Evalúa un SKILL.md contra las mejores prácticas de diseño de agent skills usando un sistema de
  6 dimensiones, clasificación de tipos (7 tipos), comparación con skills hermanos y referencias
  especializadas. Escribe la revisión en `<skills-root>/_reviews/<skill-name>-review.md`.
  En scope library o batch, delega MÚLTIPLES sub-agentes en paralelo (uno por skill) - NUNCA un solo subagente
  con múltiples skills. Audita naming, description routing, resource layout, DRY y assets, escritura directa y convenciones.
  Úsalo cuando el usuario pida revisar, auditar, evaluar, puntuar o mejorar un skill, o cuando
  detectes un SKILL.md creado o modificado y convenga verificar su calidad. No lo uses para crear
  skills desde cero, ejecutar tests, desplegar código o realizar tareas de desarrollo generales.
argument-hint: "[SKILL-PATH] [--scope single|library|batch]"
allowed-tools:
  - read
  - grep
  - exec
...
⚠️ **CRÍTICO**: Cuando el scope es `library` o `batch` y hay 2+ skills a revisar, DEBES lanzar MÚLTIPLES subagentes en paralelo (uno por skill). NUNCA lances un solo subagente que revise todos los archivos secuencialmente.
...
✅ **CORRECTO**: Lanzar N subagentes (uno por skill) en paralelo, cada uno con un solo skill asignado
...
2. Ubicaciones habituales por entorno (primer match gana salvo que el usuario sobreescriba):
   ...
   | Entorno / convención | Path típico |
   |---------------------|-------------|
...
Llena las tablas de checklists de name, description, resource layout, DRY & assets y escritura directa en [audit-checklists.md](references/audit-checklists.md). Puntúa name, description y DRY & assets por separado (1–10) según sus guías.
...
Genera hallazgos ordenados por severidad (blocker → important → optional) usando el formato en [scoring-rubric.md](references/scoring-rubric.md). Cuando overall ≥ 9, lista mejoras opcionales que no afectan puntuación.

Antes de terminar Fase B, confirma:

- Todas las tablas de checklist de auditoría llenadas antes de puntuar el cuerpo.
...
Escribe según [scoring-rubric.md § Contrato de output de revisión](references/scoring-rubric.md). Confirma que cada item de Verificación de esa sección pasa antes de terminar.

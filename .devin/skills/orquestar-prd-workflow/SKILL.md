...
description: >-
  Orquesta el workflow completo de PRD (evaluar-alcance-idea → priorizar-roadmap → evaluar-conectividad-tecnica → capturar-requerimiento → validar-viabilidad-producto → definir-usuarios → mapear-casos-uso → generar-prd) con gates de go/no-go y loop de procesamiento para múltiples funcionalidades. Genera uno o múltiples PRDs listos para planificación arquitectónica. Úsalo cuando el usuario pida crear, generar, desarrollar, idear o implementar PRDs desde una idea bruta, o definir el producto requirements document de una funcionalidad nueva. No lo usas para ejecutar skills individuales del workflow ni para planificar epics (usa orquestar-epic-workflow).
